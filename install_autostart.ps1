...
Copy-Item -LiteralPath (Join-Path $source 'usage_float_winforms.ps1') -Destination $install -Force
...
$savedShortcut = $shell.CreateShortcut($link)
if ($savedShortcut.TargetPath -ne (Join-Path $install 'start.bat')) {
    throw "Startup shortcut target is incorrect: $($savedShortcut.TargetPath)"
}
